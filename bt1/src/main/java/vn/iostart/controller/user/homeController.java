...
import vn.iostart.ultils.Constrant;
import vn.iostart.ultils.Email;

//
@WebServlet(urlPatterns = { "/home", "/login", "/register", "/forgotpass","/logout", "/waiting", "/VerifyCode" })
...
//
	...
	private void getlogout(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		if (cookies != null) {
			for (Cookie cookie : cookies) {
				if (Constrant.COOKIE_REMEMBER.equals(cookie.getName())) {
	...
	private static void homePage(HttpServletRequest req, HttpServletResponse resp)
			throws ServletException, IOException {
